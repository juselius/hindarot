...
case $with_blas in
	...
	-l* | */* | *.a | *.so | *.so.* | *.o) BLAS_LIBS="$with_blas" ;;
...
case $with_blas_dir in
	"") LIBS="$LIBS $FLIBS" ;;
	-L*) LIBS="$with_blas_dir $LIBS $FLIBS" ;;
	*) LIBS="-L$with_blas_dir $LIBS $FLIBS" ;;
esac
...
if test $acx_blas_ok = no; then
	...
	[acx_blas_ok=yes; BLAS_LIBS="-lmkl_def -lmkl_lapack -lpthread"],
	[],[-lpthread])])
fi
...
if test $acx_blas_ok = no; then
	AC_CHECK_LIB(blas, $sgemm, 
	[acx_blas_ok=yes; BLAS_LIBS="-lblas -lcraylibs"],[],[-lcraylibs])
fi
